...
import { interval, merge, fromEvent, combineLatest, Subscription } from 'rxjs';
...
export class GameComponent implements OnInit {
  @ViewChild('flappy') flappy: ElementRef;
  ...
  game() {
    ...
    const bird$ = merge(interval(300), fromEvent(document, 'keydown')).pipe(
      scan<any, any>(
        (xPos, curr) =>
          curr instanceof KeyboardEvent ? fly(xPos) : fall(xPos),
        gameSize - 1
      )
    );
    ...
    const valueOnCollisionFor = (pipes) => ({
      when: (predicate) =>
        !pipes[0][0].checked && predicate
          ? ((pipes[0][0].checked = true), 1)
          : 0,
    });

    this.subscription = combineLatest(bird$, gamePipes$)
      .pipe(
        scan<any, any>(
          (state, [bird, pipes]) => ({
            bird: bird,
            ...
            score:
              state.score +
              valueOnCollisionFor(pipes).when(pipes[0][0].y === 0),
          }),
          { lives: 3, score: 0, bird: 0, pipes: [] }
        ),
        tap((state) =>
          paint(
            updateGame(state.bird, state.pipes),
            state.lives,
            state.score,
            this.flappy.nativeElement
          )
        ),
